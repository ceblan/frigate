...
These are the MQTT messages generated by BIS-IA. The default topic_prefix is `bisia`, but can be changed in the config file.

## General BIS-IA Topics

### `bisia/available`

Designed to be used as an availability topic with Home Assistant. Possible message are:
"online": published when BIS-IA is running (on startup)
"offline": published after BIS-IA has stopped

### `bisia/restart`

Causes BIS-IA to exit. Docker should be configured to automatically restart the container on exit.

### `bisia/events`

Message published for each changed tracked object. The first message is published when the tracked object is no longer marked as a false_positive. When BIS-IA finds a better snapshot of the tracked object or when a zone change occurs, it will publish a message with the same id. When the tracked object ends, a final message is published with `end_time` set.
...
```json
{
  ...
  "before": {
    ...
    "end_time": null,
    "severity": "detection",
    "thumb_path": "/media/bisia/clips/review/thumb-front_cam-1718987129.308396-fqk5ka.webp",
    "data": {
      "detections": [
        // list of event IDs
...
### `bisia/camera_activity`

Returns data about each camera, its current features, and if it is detecting motion, objects, etc. Can be triggered by publising to `bisia/onConnect`

### `bisia/notifications/set`

Topic to turn notifications on and off. Expected values are `ON` and `OFF`.

### `bisia/notifications/state`

Topic with current state of notifications. Published values are `ON` and `OFF`.

## BIS-IA Camera Topics

### `bisia/<camera_name>/status/<role>`
...
### `bisia/<zone_name>/<object_name>/active`
...
### `bisia/<camera_name>/<object_name>/snapshot`
...
### `bisia/<camera_name>/audio/<audio_type>`
...
### `bisia/<camera_name>/classification/<model_name>`
